...
int methodArgsCount(char* descriptor) {
    ...
    aux = malloc(sizeof(char)*strlen(descriptor));
...
ClassFile* getClassFileByName(ClassList* init, char* className) {
	...
	while (cListAux != NULL) {
        ...
        classNameAux = (char*)cListAux->classFile->constantPool[classNameIndex].info.Utf8Info.bytes;
...
MethodInfo* getMethod(ClassFile cFile, char* methodName, char* methodDescriptor) {
	...
	for (i = 0; i < cFile.methodsCount; i++) {
	    methodNameAux = (char*)cFile.constantPool[cFile.methods[i].nameIndex].info.Utf8Info.bytes;
        methodDescriptorAux = (char*)cFile.constantPool[cFile.methods[i].descriptorIndex].info.Utf8Info.bytes;
...
ClassFile* isLoaded(Interpretador* interpretador, char* className) {
	...
	if (classFile == NULL ) {
		...
		classFile = loadClass(interpretador, className);
	}
	return classFile;
}
...
void fieldsInit(Interpretador *p, ClassFile classFile, Field* fields) {
	...
	for (i = 0; i < classFile_aux->fieldsCount; i++) {
		...
		descriptor = (char*)getUTF8(classFile_aux->constantPool, fieldDescriptorIndex);
		fieldNameIndex = classFile_aux->fields[i].nameIndex;
		fieldName = (char*)getUTF8(classFile_aux->constantPool, fieldNameIndex);
	...
	while (classFile_aux->superClass != 0) {
		...
		strcpy(nomeSuperClasse, (char*)getUTF8(classFile_aux->constantPool, superClassNameIndex));
		...
		if (classFile_aux != NULL ) {
			for (i = 0; i < classFile_aux->fieldsCount; i++) {
				...
				if (((classFile_aux->fields[i].accessFlags & ACC_STATIC) == 0)) {
					strcpy(fields[j].descriptor, descriptor);
					fields[j].name = fieldName;
					fields[j].fieldType.longType = 0;
					j++;
				}
